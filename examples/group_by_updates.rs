...
fn setup_test_directory() -> TempDir {
    ...
    create_file_with_date(&dir, "january_report.txt", "January report content", 0o644, 
                         Utc.with_ymd_and_hms(2023, 1, 15, 10, 0, 0).unwrap());
                         
    create_file_with_date(&dir, "january_memo.doc", "January memo content", 0o644, 
                         Utc.with_ymd_and_hms(2023, 1, 20, 14, 30, 0).unwrap());
                         
    create_file_with_date(&dir, "february_report.txt", "February report content", 0o644, 
                         Utc.with_ymd_and_hms(2023, 2, 10, 9, 15, 0).unwrap());
                         
    create_file_with_date(&dir, "february_presentation.ppt", "Presentation slides", 0o644, 
                         Utc.with_ymd_and_hms(2023, 2, 25, 16, 45, 0).unwrap());
                         
    create_file_with_date(&dir, "march_budget.xlsx", "Q1 budget spreadsheet", 0o644, 
                         Utc.with_ymd_and_hms(2023, 3, 5, 11, 20, 0).unwrap());
                         
    create_file_with_date(&dir, "march_notes.txt", "Meeting notes", 0o644, 
                         Utc.with_ymd_and_hms(2023, 3, 18, 13, 10, 0).unwrap());
    
    // Create some image files from different days
    create_file_with_date(&dir, "vacation1.jpg", &vec![0; 10000].into_iter().map(|_| b'X').collect::<Vec<u8>>(), 0o644, 
                         Utc.with_ymd_and_hms(2023, 7, 1, 10, 0, 0).unwrap());
                         
    create_file_with_date(&dir, "vacation2.jpg", &vec![0; 12000].into_iter().map(|_| b'X').collect::<Vec<u8>>(), 0o644, 
                         Utc.with_ymd_and_hms(2023, 7, 1, 14, 30, 0).unwrap());
                         
    create_file_with_date(&dir, "vacation3.jpg", &vec![0; 15000].into_iter().map(|_| b'X').collect::<Vec<u8>>(), 0o644, 
                         Utc.with_ymd_and_hms(2023, 7, 2, 9, 0, 0).unwrap());
    
    // Create files with different hour timestamps
    create_file_with_date(&dir, "morning_log.txt", "Morning log entry", 0o644, 
                         Utc.with_ymd_and_hms(2023, 8, 15, 8, 0, 0).unwrap());
                         
    create_file_with_date(&dir, "afternoon_log.txt", "Afternoon log entry", 0o644, 
                         Utc.with_ymd_and_hms(2023, 8, 15, 14, 0, 0).unwrap());
                         
    create_file_with_date(&dir, "evening_log.txt", "Evening log entry", 0o644, 
...
fn create_file_with_date(dir: &TempDir, path: &str, content: &(impl AsRef<[u8]> + ?Sized), mode: u32, date: DateTime<Utc>) {
    ...
    #[cfg(unix)]
    {
        ...
        let secs = date.timestamp() as i64;
        let nsecs = 0;
...
fn rename_by_month(dir_path: &PathBuf) {
...
fn organize_by_day(dir_path: &PathBuf) {
...
fn prefix_by_hour(dir_path: &PathBuf) {
...
fn list_all_files(dir_path: &PathBuf) {
