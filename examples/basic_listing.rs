...
fn setup_test_directory() -> TempDir {
    ...
    create_file(
        &dir,
        "image.jpg",
        &vec![0; 2000].into_iter().map(|_| b'X').collect::<Vec<u8>>(),
    );
...
fn basic_listing(dir_path: &Path) {
    let args = Args {
...
fn long_format_listing(dir_path: &Path) {
    let args = Args {
        path: dir_path.to_path_buf(),
        long_view: true,
        show_total: true,
...
fn recursive_listing(dir_path: &Path) {
    let args = Args {
        path: dir_path.to_path_buf(),
        long_view: false,
        show_total: false,
...
fn sorted_listing(dir_path: &Path) {
