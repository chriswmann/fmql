...
fn setup_test_directory() -> TempDir {
    ...
    create_file(
        &dir,
        "image1.jpg",
        &vec![0; 1500].into_iter().map(|_| b'X').collect::<Vec<u8>>(),
    );
    create_file(
        &dir,
        "image2.jpg",
        &vec![0; 2500].into_iter().map(|_| b'X').collect::<Vec<u8>>(),
    );
    create_file(
        &dir,
        "image3.png",
        &vec![0; 3500].into_iter().map(|_| b'X').collect::<Vec<u8>>(),
    );
    ...
    create_file(
        &dir,
        "images/vacation.jpg",
        &vec![0; 4000].into_iter().map(|_| b'X').collect::<Vec<u8>>(),
    );
    create_file(
        &dir,
        "code/main.rs",
...
fn filter_by_pattern(dir_path: &Path) {
...
fn show_hidden_files(dir_path: &Path) {
...
fn group_by_extension(dir_path: &Path) {
    let args = Args {
        path: dir_path.to_path_buf(),
        long_view: false,
        show_total: false,
...
fn group_by_type(dir_path: &Path) {
    let args = Args {
        path: dir_path.to_path_buf(),
        long_view: false,
        show_total: false,
...
fn group_by_size(dir_path: &Path) {
    let args = Args {
        path: dir_path.to_path_buf(),
        long_view: false,
        show_total: false,
...
fn output_formats(dir_path: &Path) {
    let formats = [
